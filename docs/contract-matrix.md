...
| contract_id | top_level_source | rule | visible_to_user | platform | required_tests | notes |
| --- | --- | --- | --- | --- | --- | --- |
| OB-001 | v1:执行模型 + 状态协议 | Long-running execution must keep one progress card and refresh within bounded silence (no 30s card spam). Progress updates and heartbeat updates must keep the same running-card visual style to avoid status-card drift. | both | cross-platform | persistence, presentation, runtime_compat | Proof: `runtime/persistence/heartbeat:*`, `runtime/protocol/presentation:*`; known stderr router noise should not overwrite the last visible status hint (`runtime/persistence/progress: router stderr noise does not overwrite the last visible status hint`); progress/heartbeat style consistency (`runtime/protocol/presentation: progress and running updates keep the same running card style`). |
