...
- 继续当前工作的主路径是直接回复下一步给 Codex；显式续写沿用原生命名 `resume`：`/resume [--model <model>] [--reasoning <level>] <prompt>`，兼容 `/codex resume ...`
- 原生 `codex resume` 已支持 session id / thread name / `--last` 等选择模式；bridge 主路径明确只保留 prompt 续写，不纳入选择面；task 激活后普通文本仍是默认续写路径；如需显式续写，推荐用 `/resume <prompt>`
...
- 显式新任务优先使用短命令：`/run --cd <path> [--model <model>] [--reasoning <level>] <prompt>`，兼容 `/codex --cd ...`
...
- 在 paired bridge 私聊表面，历史顶层 `/new` / `/reset` 已关闭，不再作为会话切换入口；显式新任务走 `/run --cd ...` 或兼容 `/codex --cd ...`，显式续写走 `/resume ...` 或兼容 `/codex resume ...`

## Inbound Input Semantics

> 这是当前已批准的目标语义，用于约束后续 inbound multimodal 落地；当前 runtime 还没有实现这条媒体归一化路径。

- 普通 Feishu 消息在该目标语义下仍默认归 `Codex`
- 文本、图片、音频、视频、文件都应被视为同一条普通用户输入的可能组成部分
- bridge 只应把飞书媒体外壳规范化成受控附件上下文，不新增媒体专用命令面
- 媒体下载失败时应 fail closed：文本仍进入任务，失败原因只保留最小上下文提示
...
- run 完成或失败后，task 默认回到 `awaiting_input`；这表示当前这一轮结束，但同一 task lane 仍可继续
...
- 当前 `deliverables` 只接受：
  ...
  - 外部 `link`
- HTML 页面、web preview 等本地可打开页面按普通 `file` 产物回传；若误声明为 `html`，bridge 会规范为文件回传，不新增独立 `html` 传输类型
...
- 成功产物当前通过原生 Feishu 消息回到同一 origin；`.html` 页面和 `.svg` 这类向量图会降级为文件回传，而不是按图片渲染
...
- 第一阶段兼容性实验新增四个 allowlisted 短命令：`/help`、`/doctor`、`/resume`、`/run`；既有 `/codex ...` 继续兼容。
- 显式命令面优先贴近原生 `Codex CLI` 的概念与参数名。
- 继续当前工作时，普通文本回复是默认路径；显式续写保留原生命名 `resume`，但 bridge 主路径不开放 session id / thread name / `--last` 等选择面；推荐 `/resume <prompt>`，兼容 `/codex resume <prompt>`。
- 当前显式启动 / 续写只收口三类启动参数：`--cd`、`--model`、`--reasoning`。
- 当前显式启动 / 续写额外支持原生权限参数：`--sandbox`、`--ask-for-approval`。
- `/run --cd ... <prompt>` 与兼容 `/codex --cd ... <prompt>` 的语义是“在指定目录启动一轮 Codex 任务”，不是“仅切目录不执行”；若只写“进入目录/切换目录”这类空提示词，也会按普通任务执行并等待模型返回。
...
- 若当前 DM 已记住 `full_access`，显式 `/run ... --sandbox workspace-write ...`、`/resume --sandbox workspace-write ...` 或兼容 `/codex ... --sandbox workspace-write ...` 是当前最小心智的反向操作：本轮按普通沙箱启动，并清掉该 DM 记住的 `full_access` 默认值。
- `--sandbox` 决定沙箱边界；`--ask-for-approval` 只决定审批策略。两者不是同义词，`--ask-for-approval never` 不等于 `danger-full-access`。
- 即使普通文本提到 `~/.openclaw`、`~/.codex`、模型名或思考等级，只要它不是显式短命令或 `/codex ...` 启动面，也仍归 `Codex` lane。
- bridge 只在显式短命令、兼容 `/codex ...` 或自有审批 / 控制面闭环里做最薄 gate，不接管普通发给 Codex 的内容语义。
- 上游会话 reset 的语义也只接受官方 hook 信号；bridge 不对普通文本额外做 reset 语义猜测。
- `/doctor` 当前输出真实健康摘要：`Codex CLI`、`bwrap`、隔离 Feishu 凭据、gateway，再加一条具体下一步；兼容 `/codex doctor`。
- 未知 `/codex <subcommand>` 返回简短、native-first 的提示，不再回退到旧帮助页。
- 历史 `/codex` 兼容命令已关闭执行；它们只返回关闭提示，不再承担迁移、恢复、状态或审批入口语义。
- 当前文档明确对外的显式入口只有：

```text
/run --cd <path> --model <model> --reasoning <level> <prompt>
/run --cd <path> --sandbox danger-full-access <prompt>
/resume --model <model> --reasoning <level> <prompt>
/doctor
/help
```
...
```text
/run --cd /path/to/workspace 帮我先执行 pwd 并概览当前目录结构
```
...
- 若当前 DM 已记住 `full_access`，显式 `/run ...` 新任务与 `/resume ...` 续写默认都以高权限 lane 启动；兼容 `/codex ...` 同样适用；普通文本仍保持原有 lane 归属，不因此被 bridge 接管
- 若当前 DM 已记住 `full_access`，但显式命令同时给出更保守的原生沙箱参数（当前为 `--sandbox workspace-write`），则以该显式原生参数为准，并把该 DM 默认权限切回 `normal`
...
- 普通文本：允许，创建新 task
- `/doctor` 或 `/codex doctor`：目标命令；用于 bridge 真实健康摘要

### `awaiting_input`

- 普通文本：允许，续到同一 task 的下一次 run
- 若该 task 带“上一轮执行中断”语义：普通文本仍允许，默认按自然语言继续
- `/resume <prompt>` 或 `/codex resume <prompt>`：允许，但仅作为显式续写 fallback
- `/doctor` 或 `/codex doctor`：目标命令；用于 bridge 真实健康摘要
...
- `/help`、`/doctor`、`/resume`、`/run` 是第一阶段兼容性实验短命令；其它非 `/codex` slash 命令仍不被 bridge claim
- 高风险审批对 `codex task` 当前会同时写入 DM-scoped `full_access`：它既批准这一次获批 run，也把当前 paired DM 的默认启动权限切到高权限，直到显式 `/run ... --sandbox workspace-write ...`、`/resume --sandbox workspace-write ...`、兼容 `/codex ... --sandbox workspace-write ...` 降权或 reset
...
- 除 `/help`、`/doctor`、`/resume`、`/run` 这四个兼容性实验短命令外，非 `/codex` 的 slash 命令不会被 bridge claim
...
- 对用户提示为“上一轮执行中断，请直接回复下一步给 Codex”
- 若中断原因可判定为“桥接器所在 service 被它自己重启”，则提示更具体的恢复语义，而不是只给通用中断文案
- 当前不根据“长时间无心跳”自动改写 task 状态；无心跳只用于观测，不用于推断执行已死

## 执行后端（实验）

- 当前支持内部执行后端选择：`executionBackend=cli|ws`。
- 默认 `cli`；未显式开启前，行为与当前主路径一致。
- `ws` 后端仅改变执行承载通道，不改变命令面、审批边界、任务状态语义。
- 当 `ws` 启动失败：
  - 若 `wsBackendAutoFallbackToCli=true`，会自动回落到 `cli`；
  - 否则 fail-closed，run 失败并返回显式错误。
- 相关配置：
  - `executionBackend`
  - `wsBackendUrl`
  - `wsBackendAuthTokenEnv`
  - `wsBackendAutoFallbackToCli`
