...
- 继续当前工作的主路径是直接回复下一步给 Codex；显式续写沿用原生命名 `resume`：`/codex resume [--model <model>] [--reasoning <level>] <prompt>`
- 原生 `codex resume` 已支持 session id / thread name / `--last` 等选择模式；bridge 主路径明确只保留 prompt 续写，不纳入选择面；task 激活后普通文本仍是默认续写路径；如需显式续写，再用 `/codex resume <prompt>`
...
- 继续当前工作时，普通文本回复是默认路径；显式续写保留原生命名 `resume`，但 bridge 主路径不开放 session id / thread name / `--last` 等选择面；如需显式续写，再用 `/codex resume <prompt>`。
...
- `/codex resume <prompt>`：允许，但仅作为显式续写 fallback
- `/codex doctor`：目标命令；用于 bridge 真实健康摘要
